...
if (!dir.exists(file.path(paste(plotpre_fig, "epicode46_ribbons_supp", sep="")))){
  dir.create(file.path(paste(plotpre_fig, "epicode46_ribbons_supp", sep="")))
}
...
# For hospinc VSEV, bring in the region specific ribbons and draw them in blue. 
# Make the panel: comm_inc sev, comm_inc vsev, hosp_inc sev on top
# then the three regions on the bottom for hosp_inc vsev 
# Or, left/right.
...
burden_ribbon_df = burden_ribbon %>% cubelyr::as.tbl_cube(met_name="Val") %>% as_tibble %>% 
                    ...
                    mutate(outcome = factor(outcome, 
                           levels = c("comm_sev", "comm_vsev", "hosp_sev" , "hosp_vsev"),
                           labels = c("Severe cases", "Very severe cases", 
                                      "Severe hospitalizations", "Very severe hospitalizations"))) %>% 
...
png(filename = paste(plotpre_fig, "epicode46_ribbons_supp/outcome_ribbons_sev.png", sep=""),
    width = 7.5, height = 7.5, units = 'in', res=500,
    family = "Arial")
grid.newpage()
print(grid.draw(p))
dev.off()
